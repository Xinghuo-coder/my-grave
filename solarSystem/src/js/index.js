...
var starArray = [];
var focusedPlanet = null; // 当前聚焦的星球
var isZoomedIn = false; // 是否已放大
var earthMesh = null; // 地球网格对象
var earthHotspots = null; // 地球热点系统
var earthHotspotsUI = null; // 地球热点UI
// 选项
initThree();
initCamera();
initScene();
...
function initEarth() {
  ...
  var geometry = new THREE.SphereGeometry(1, 64, 64);
  // 使用MeshPhongMaterial以支持更好的光照和高亮效果
  var material = new THREE.MeshPhongMaterial({ 
    color: 0xffffff,
    emissive: 0x000000, // 自发光颜色（用于高亮）
    shininess: 10
  });
...
function initEarthHotspots() {
  ...
  try {
    // 初始化UI管理器
    earthHotspotsUI = new EarthHotspotsUI();
    
    // 初始化热点系统
    earthHotspots = new EarthHotspots(
      earthMesh,
      camera,
      renderer.domElement
    );
    ...
    // 创建热点
    earthHotspots.createHotspots();
    
    // 更新统计信息
    const stats = earthHotspots.getStatistics();
    earthHotspotsUI.updateStatistics(stats);
    
    console.log('✅ 地球热点系统初始化成功');
    console.log(`📊 总区域数: ${stats.totalHotspots.toLocaleString()}`);
    console.log(`🌍 地球半径: ${stats.earthRadius.toLocaleString()} m`);
    console.log(`📐 目标面积: ${stats.targetArea} m²`);
    
  } catch (error) {
    console.error('❌ 地球热点系统初始化失败:', error);
  }
}