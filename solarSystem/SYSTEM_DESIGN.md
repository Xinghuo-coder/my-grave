...
地块分配：
├── 地块ID（必填）- 必须在用户可用范围内（中间 90%）
├── 地块编号（自动生成）
└── 注意：前 5% 和后 5% 的地块被保留，不对用户开放
...
| is_reserved | BOOLEAN | 是否为保留地块（前5%或后5%） |
| created_at | TIMESTAMP | 创建时间 |
...
系统为了管理和扩展而**保留地块编号的前 5% 和后 5%**，这些地块不对普通用户开放。

### 保留范围
- **总地块数**: 127,000,000 个
- **前 5% 保留**: 1 - 6,350,000
- **用户可用**: 6,350,001 - 120,650,000（中间 90%）
- **后 5% 保留**: 120,650,001 - 127,000,000

### 限制规则
- 用户只能在**中间 90% 的地块**上创建坟墓
- 尝试访问保留地块会返回 **403 Forbidden** 错误
- 管理员可以在保留地块上创建特殊纪念区域

详细信息见 [RESERVED_BLOCKS_GUIDE.md](RESERVED_BLOCKS_GUIDE.md)
